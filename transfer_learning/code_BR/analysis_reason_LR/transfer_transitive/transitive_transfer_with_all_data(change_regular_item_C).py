...
# csv_path
csv_path = '/home/huxinhou/WorkSpace_BR/transfer_learning/result/analysis_reason_LR/transfer_transitive(change_regular_item)/'
...
for data_num in range(1, 6):
    ...
    auc_dataframe = pd.DataFrame(index=disease_list.iloc[:, 0], columns=C_list)
    ...
    for disease_num in range(len(disease_list)):
        ...
        for C in C_list:
            ...
            while i < 10:
                ...
                y_train = target_train_meaningful_sample['Label']
                ...
                lr_DG_ran_smp = LogisticRegression(n_jobs=-1 , C=C)
                try:
            ...
            auc_dataframe.loc[disease_list.iloc[disease_num , 0], C] = round(np.mean(auc_list), 3)
            auc_mean_dataframe.loc[disease_list.iloc[disease_num , 0], C] += np.mean(auc_list)
