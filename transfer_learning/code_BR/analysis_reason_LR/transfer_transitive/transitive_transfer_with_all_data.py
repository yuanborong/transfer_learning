...
# csv_path
csv_path = '/home/huxinhou/WorkSpace_BR/transfer_learning/result/analysis_reason_LR/transfer_transitive_multiple/'
...
multiple_weight_size = [1 , 5 , 10 , 20 , 100]

# 创建一个5折交叉平均的df
auc_mean_dataframe = pd.DataFrame(np.ones((len(disease_list), len(multiple_weight_size))) * 0, index=disease_list.iloc[:, 0],
                                  columns=multiple_weight_size)
...
for data_num in range(1, 6):
    ...
    auc_dataframe = pd.DataFrame(index=disease_list.iloc[:, 0], columns=multiple_weight_size)
