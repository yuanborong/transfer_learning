...
abs_transfer_csv_path = 'abs_transfer_transitive\\'
abs_without_transfer_csv_path = 'abs_large_group_without_transfer\\'
...
for data_num in range(1, 6):
    ...
    transfer_abs_csv_name = 'abs_middle_model_coef_{}.csv'.format(data_num)
    without_transfer_abs_csv_name = 'abs_large_group_model_coef_{}.csv'.format(data_num)
    ...
    # df_abs_transfer = pd.DataFrame(np.ones((, len(large_group_list))) * 0 , index=large_group_list ,
                                        # columns = large_group_list)
    df_abs_transfer = df_transfer.abs()
    df_abs_without_transfer = df_without_transfer.abs()
    ...
    df_abs_transfer.to_csv(csv_path + abs_transfer_csv_path + transfer_abs_csv_name)
    df_abs_without_transfer.to_csv(csv_path + abs_without_transfer_csv_path + without_transfer_abs_csv_name)
